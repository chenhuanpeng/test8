...
package com.aeye.aeaimb.auth.endpoint;
...
import com.aeye.aeaimb.mkpb.feign.RemoteMkpbClientDetailsService;
import com.aeye.aeaimb.mkpb.vo.sys.TokenVo;
...
import com.aeye.aeaimb.auth.support.handler.CdssAuthenticationFailureEventHandler;
...
import com.aeye.aeaimb.common.security.util.OAuth2EndpointUtils;
import com.aeye.aeaimb.common.security.util.OAuth2ErrorCodesExpand;
import com.aeye.aeaimb.common.security.util.OAuthClientException;
...
public class CdssTokenEndpoint {

	private final HttpMessageConverter<OAuth2AccessTokenResponse> accessTokenHttpResponseConverter = new OAuth2AccessTokenResponseHttpMessageConverter();

	private final AuthenticationFailureHandler authenticationFailureHandler = new CdssAuthenticationFailureEventHandler();

	private final OAuth2AuthorizationService authorizationService;

	private final RemoteMkpbClientDetailsService clientDetailsService;

	private final RedisTemplate<String, Object> redisTemplate;

	private final CacheManager cacheManager;

	// sso用户名密码,先暂时固定，后台添加一个相应的用户和密码，后面看怎么映射 @todo
	private  String userName = "admin";
	private  String password = "YasPSoLO+cM=";
	...
	private static final String SERVICE_NAME = "aeaimb-gateway";

	@GetMapping("/getZtToken")
	public R<String> getZtToken(String username) {
		log.info("username:" + username);
		//根据username获取映射的知识中台用户 @todo

		Map<String, String> serviceIpPort = getServiceIpPort(SERVICE_NAME);
		if (StrUtil.isNotBlank(serviceIpPort.get("ip")) && StrUtil.isNotBlank(serviceIpPort.get("port"))) {
			String urlBuilder = "http://" +
					(serviceIpPort.get("ip"))+
					":" +
					serviceIpPort.get("port") +
					"/kauth/oauth2/token?" +
					encodeParam("grant_type", "password") +
					"&randomStr=ee0b890b-ff44-4e93-a560-bdd6687a672b&code=8&scope=server&" +
					encodeParam("username", userName);
			HttpRequest request = HttpUtil.createPost(urlBuilder)
					.header(HttpHeaders.AUTHORIZATION, "Basic cGlnOnBpZw==")
					.header(HttpHeaders.CONTENT_TYPE, "application/x-www-form-urlencoded")
					.body(buildRequestBody(password));
			HttpResponse execute = request.execute();
			if (execute.isOk()) {
				String body = execute.body();
				JSONObject jsonObject = JSONObject.parseObject(body);
				return  R.ok((String)jsonObject.get("access_token"));
			}
		}
		return  R.failed();
	}
	...
	private static String buildRequestBody(String password) {
