...
package com.aeye.aeaimb.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.aeye.aeaimb.common.xss.config.CdssXssProperties;
import com.aeye.aeaimb.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final CdssXssProperties properties;

	public DefaultXssCleaner(CdssXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(CdssXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		CdssXssProperties.Mode mode = properties.getMode();
		if (CdssXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (CdssXssProperties.Mode.validate == mode) {
			// 校验
